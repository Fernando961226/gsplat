    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        for (int t = 0; (t < batch_size) && !done; ++t) {
            ...
            const float sigma = 0.5f * (conic.x * delta.x * delta.x +
                                        conic.z * delta.y * delta.y) +
                                conic.y * delta.x * delta.y;
            // TODO(fernando): the following equation makes such that the maximun value is 1 
            const float alpha = max(-1.f, min(1.f, opac * __expf(-sigma))); 

            // TODO(fernando): need to allow negatives
